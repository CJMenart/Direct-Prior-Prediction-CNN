...
def mnist_expr(gpu=None):
	...
	for iter in range(100001):
		...
		if iter % ITER_PER_EVAL == 0:
			print(iter)
			eval(CLASS_NET_TYPES,nets,eval_data,eval_labels,inputs,truths,is_train,variance,sess)
...
def eval(CLASS_NET_TYPES,nets,eval_data,eval_labels,inputs,truths,is_train,variance,sess):
	acc_counts = {}
	in_spread_record = {}
	out_spread_record = {}
	...
	in_spreads = {}
	out_spreads = {}
	for type in ALL_NET_TYPES+['CLUST','SYS']:
	...
	file_print(','.join(["%.5f"%accs[type] for type in ALL_NET_TYPES+['CLUST','SYS']]),'err.csv')
	file_print(','.join(["%.5f"%in_spreads[type] for type in ALL_NET_TYPES]) + ','.join(["%.5f"%out_spreads[type] for type in ALL_NET_TYPES]),'variance.csv')
