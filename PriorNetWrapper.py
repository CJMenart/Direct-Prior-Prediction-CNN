...
class PriorNetWrapper:
	...
	def __init__(self,net_opts):
		...
		self.remap_base_prob = tf.placeholder(DAT_TYPE,[None,None,net_opts['numLabels']])		
		self.map_mat = tf.placeholder(DAT_TYPE,[net_opts['numLabels'],net_opts['numLabels']])
		'''
		...
		'''
		self.class_frequency = tf.placeholder(DAT_TYPE,[1,net_opts['numLabels']])
		...
		if net_opts['base'] == 'resnet_v1':
			with slim.arg_scope(resnet_v1.resnet_arg_scope()) as scope:
				resnet_out, _ = resnet_v1.resnet_v1_152(self.inputs,is_training=False if net_opts['fixBN'] else self.isTrain,global_pool=True)
				if DEBUG:
					print('resnet_out:')
			...
			with slim.arg_scope(resnet_v2.resnet_arg_scope()) as scope:
				resnet_out, _ = resnet_v2.resnet_v2_152(self.inputs,is_training=False if net_opts['fixBN'] else self.isTrain,global_pool=True)
				if DEBUG:
					print('resnet_out:')
					print(resnet_out.shape.as_list())
				self.base_net = tf.squeeze(resnet_out,[1,2])
			...
			with slim.arg_scope(inception_arg_scope()):
				logits, end_points = inception.inception_v4(self.inputs, num_classes=None, is_training=False if net_opts['fixBN'] else self.isTrain)
		...
		if net_opts['remappingLossWeight'] > 0:
		...
		if net_opts['remappingLossWeight'] > 0:
			self.loss = self.direct_prior_loss + net_opts['remappingLossWeight']*self.remapping_loss
		else:
			self.loss = self.direct_prior_loss
		
		if net_opts['isDistribution']:
	...
	def prior_predictor(self,net_opts):
		...
		out_chann = net_opts['widthHidLayers']
		for lay in range(net_opts['numHidLayers']):
			with tf.variable_scope('fc_%d' % lay) as scope:
				...
				activation = tf.nn.dropout(activation,net_opts['dropProb'])
				in_feat = activation
				in_chann = in_feat.shape.as_list()[-1]
	...
	def loadWeights(self,init_weight_fname,sess,net_opts):
	...
	def expand_target(self,target,net_opts): 
		target = tf.one_hot(target,net_opts['numLabels']+1,dtype=DAT_TYPE,axis=3)
		target = target[:,:,:,1:]
		return target
